...
namespace Lewis.SST
{
    ...
    public class SQLSchemaTool
    {
		...
		public static string SerializeDB(string SQLServer, string DBName, string UID, string PWD, string SQLfile, bool Translate, 
            bool Primary, object threaded, byte objectsToSerialize, string CustomXSLT, string delimTableNames)
		{
            _threaded = threaded;
            string _serverDB = SQLServer + ":" + DBName;
            string outputFile = string.Format(_OUTPUTFILE, SQLServer.Replace("\\", "_").Replace(":", "-"), DBName.Replace("\\", "_").Replace(":", "-"));
			try
			{
				...
				if (_connections != null && _connections.Count > 0)
				{
					...
					dr.ItemArray = new object[] { DBName, DateTime.Now.ToShortDateString(), DateTime.Now.ToShortTimeString() } ;
					dt.Rows.Add(dr);
                    ...
                    // add the following to the xmldoc
                    //<?xml version="1.0"?>
                    //<?xml-stylesheet type="text/xsl" href="Create_DB_Schema.xslt"?>
                    //XmlDeclaration declare = xmlDoc.CreateXmlDeclaration("1.0", "UTF-8", "yes");
                    //XmlProcessingInstruction procInstruct = xmlDoc.CreateProcessingInstruction("xml-stylesheet", "type='text/xsl' href='Create_DB_Schema.xslt'");
                    //xmlDoc.PrependChild(procInstruct);
                    //xmlDoc.PrependChild(declare);

...
<xsl:if test="position()=last()"><xsl:text>
	...
	<xsl:for-each select="COLUMN[Action='Add' and isComputed=0 and isRowGuidCol=0 and contains(Base_Type,'text')!=1 and ((isNullable=0 and string-length(Default_Name)=0) or (string-length(Collation)>0 and Type!=Base_Type))]">
		<xsl:if test="position()=1">
...
<xsl:text>
	...
	<xsl:for-each select="COLUMN[Action='Alter' and isComputed=0 and isRowGuidCol=0 and contains(Base_Type,'text')!=1 and ((string-length(Collation)>0 and Type!=Base_Type) or (isNullable=0 and string-length(Default_Name)=0))]">
		<xsl:if test="position()=1">
...
<!-- template to output Table Column if table is being altered -->
	<xsl:template name="_Column" match="COLUMN">
		...
		<xsl:if test="$_action='Alter' and contains($_type,'text')!=1">ALTER COLUMN </xsl:if><xsl:if test="$_action='Drop'">DROP COLUMN </xsl:if>[<xsl:value-of select="Column_Name"/>] <xsl:if test="$_action!='Drop'"><xsl:if test="string-length(Collation)>0">[<xsl:value-of select="$_btype"/>] </xsl:if><xsl:if test="string-length(Collation)=0">[<xsl:value-of select="$_type"/>] </xsl:if> <xsl:if test="string-length(Collation)>0 and contains($_btype,'char')"> (<xsl:value-of select="Length"/>) </xsl:if> <xsl:if test="$_isIdent=1"> Identity (<xsl:value-of select="Seed"/> ,<xsl:value-of select="Increment"/>) </xsl:if> <xsl:if test="string-length(Collation)>0"> COLLATE </xsl:if><xsl:value-of select="Collation"></xsl:value-of><xsl:if test="(string-length(Default_Name)>0 and isNullable=0) or ($_action='Alter' and isNullable=0)"> NOT NULL </xsl:if><xsl:if test="(string-length(Default_Name)=0 and $_action='Add') or isNullable=1"> NULL </xsl:if><xsl:if test="string-length(Default_Name)>0 and $_action='Add'"> DEFAULT <xsl:value-of select="Default_Value"/></xsl:if></xsl:if>
	</xsl:template> 
...
<!-- template to drop original table indexes - non unique and non pk -->
	<xsl:template name="DROP_INDEX">
		...
		<xsl:variable name="_keys" select="index_keys"/>
		<xsl:if test="contains($_desc, 'primary key')!=1 and contains($_desc, 'unique key')!=1">
<xsl:if test="contains($_comment, 'no')!=1">
...
<xsl:if test="contains($_comment, 'no')!=1 and string-length($_name)>0">END
GO
</xsl:if>
		</xsl:if>
		<xsl:if test="contains($_desc, 'primary key') or contains($_desc, 'unique key')">
...
<!-- functional equivalent of starts with / replace substring method -->
	<xsl:template name="StartsWithReplace">
		...
		<xsl:param name="once" />
		<xsl:choose>
			<xsl:when test="starts-with(normalize-space($stringIn),$substringIn) and $once=0">
				<xsl:value-of select="concat(substring-before($stringIn,$substringIn),$substringOut)" /> 
				<xsl:call-template name="SubstringReplace">
					<xsl:with-param name="stringIn" select="substring-after($stringIn,$substringIn)" /> 
					<xsl:with-param name="substringIn" select="$substringIn" /> 

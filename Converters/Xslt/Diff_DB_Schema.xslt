...
-- Diff Name: <xsl:value-of select="Name"/>
...
<xsl:if test="position()=last()"><xsl:text>
	...
	<xsl:for-each select="COLUMN[Action='Add' and isComputed=0 and isRowGuidCol=0 and ((isNullable=0 and string-length(Default_Name)=0) or (string-length(Collation)>0 and Type!=Base_Type))]">
		<xsl:if test="position()=1">
...
<xsl:text>
	...
	<xsl:for-each select="COLUMN[Action='Alter' and isComputed=0 and isRowGuidCol=0 and ((string-length(Collation)>0 and Type!=Base_Type) or (isNullable=0 and string-length(Default_Name)=0))]">
		<xsl:if test="position()=1">
...
<!-- template to output non-calculated CREATE TABLE Column -->
	<xsl:template name="_CreateColumn" match="COLUMN[isComputed=0]">
		...
		[<xsl:value-of select="Column_Name"/>] [<xsl:value-of select="$_type"/>] <xsl:if test="contains($_type, 'char') and Length=-1">(MAX) </xsl:if><xsl:if test="(contains($_type, 'char') or contains($_type,'numeric') or contains($_type,'decimal')) and Length!=-1">(<xsl:value-of select="Length"/><xsl:if test="contains($_type,'numeric') or contains($_type,'decimal')">,<xsl:value-of select="Scale"/></xsl:if>) </xsl:if> <xsl:if test="$_isIdent=1"> Identity (<xsl:value-of select="Seed"/> ,<xsl:value-of select="Increment"/>)</xsl:if><xsl:if test="NotforRepl=1"> NOT FOR REPLICATION</xsl:if><xsl:if test="string-length(Collation)>0"> COLLATE </xsl:if> <xsl:value-of select="Collation"/><xsl:if test="isNullable=0"> NOT</xsl:if> NULL<xsl:if test="isRowGuidCol=1"> ROWGUIDCOL</xsl:if><xsl:if test="string-length(Default_Name)>0"> DEFAULT <xsl:value-of select="Default_Value"/></xsl:if><xsl:if test="position()!=last()">,</xsl:if>
	</xsl:template>
...
<!-- template to output Table Column if table is being altered -->
	<xsl:template name="_Column" match="COLUMN">
      ...
      <xsl:if test="$_action='Add'"> [<xsl:value-of select="Column_Name"/>] </xsl:if><xsl:if test="$_action='Alter' and not(contains($_type,'text'))">ALTER COLUMN  [<xsl:value-of select="Column_Name"/>] </xsl:if><xsl:if test="$_action='Drop'">DROP COLUMN  [<xsl:value-of select="Column_Name"/>] </xsl:if><xsl:if test="$_action!='Drop' or ($_action='Add' and not(contains($_type,'text')))"><xsl:if test="string-length(Collation)>0"> [<xsl:value-of select="$_btype"/>] </xsl:if><xsl:if test="string-length(Collation)=0"> [<xsl:value-of select="$_type"/>] </xsl:if><xsl:if test="(string-length(Collation)>0 and contains($_btype,'char')) or contains($_btype,'numeric') or contains($_btype,'decimal')"> (<xsl:value-of select="Length"/><xsl:if test="contains($_btype,'numeric') or contains($_btype,'decimal')">,<xsl:value-of select="Scale"/></xsl:if>) </xsl:if><xsl:if test="$_isIdent=1"> Identity (<xsl:value-of select="Seed"/> ,<xsl:value-of select="Increment"/>) </xsl:if><xsl:if test="string-length(Collation)>0"> COLLATE </xsl:if> <xsl:value-of select="Collation"/> <xsl:if test="(string-length(Default_Name)>0 and isNullable=0) or ($_action='Alter' and isNullable=0)"> NOT NULL </xsl:if><xsl:if test="(string-length(Default_Name)=0 and $_action='Add') or isNullable=1"> NULL </xsl:if><xsl:if test="string-length(Default_Name)>0 and $_action='Add'"> DEFAULT <xsl:value-of select="Default_Value"/> </xsl:if></xsl:if>
	</xsl:template> 
...
<!-- template to fix Table Column that are Not Null and does not have a default value, or for UDDTs for the ADDed/Altered columns -->
  ...
  ALTER COLUMN [<xsl:value-of select="Column_Name"/>] [<xsl:value-of select="Type"/>] <xsl:if test="contains(Base_Type,'char') and Base_Type=Type and Length=-1"> (MAX) </xsl:if><xsl:if test="((contains(Base_Type,'char') and Base_Type=Type) or contains(Base_Type,'numeric') or contains(Base_Type,'decimal')) and Length>0"> (<xsl:value-of select="Length"/><xsl:if test="contains(Base_Type,'numeric') or contains(Base_Type,'decimal')">,<xsl:value-of select="Scale"/></xsl:if>) </xsl:if><xsl:if test="string-length(Collation) > 0"> COLLATE </xsl:if> <xsl:value-of select="Collation"/> <xsl:if test="isNullable=0"> NOT NULL</xsl:if><xsl:text>
</xsl:text>
    </xsl:if>
    <xsl:if test="contains(Base_Type,'text')">
      print 'WARNING: It is currently not possible alter the text column ''<xsl:value-of select="Column_Name"/>'' using this tool, process will require exmination of field change and possible extra scripting'
      <xsl:text>  
</xsl:text>
    </xsl:if>
  </xsl:template>		
